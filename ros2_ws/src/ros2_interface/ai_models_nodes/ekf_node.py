...
class EKFNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("initial_covariance", [0.5, 0.5, 0.5])
        self.declare_parameter("R_gnss", [0.25, 0.25])
        self.declare_parameter("R_mag", [0.01])

        # init EKF
        x0 = self.get_parameter("initial_state").value
        P0 = self.get_parameter("initial_covariance").value
        R_gnss_mtx = np.diag(self.get_parameter("R_gnss").value)
        R_mag_val = np.array(self.get_parameter("R_mag").value)
        self.ekf = EKFAlgorithm(x0, P0)
        self.ekf.set_observation_noise("gnss", R_gnss_mtx)
        self.ekf.set_observation_noise("magnetometer", R_mag_val)
    ...
    def imu_callback(self, msg: Imu):
        ...
        v = 1.0  # Assume value from other source
